...
START_TOKEN = '<s>'
END_TOKEN = '</s>'
TRAIN_FILE = '/home/ago109/IdeaProjects/Delta-RNN-Theano/text.txt'
VAL_FILE = '/home/ago109/IdeaProjects/Delta-RNN-Theano/text.txt'
...
def train_model(seed = 12345,model='rnn',batch_size=50, n_h=50, n_epochs=40, updater='sgd', lr = 0.002,
    ...
    useLN=False, drop_p=0., grad_clip=0, patience=1):
    ...
    if updater == 'sgd': # use classical SGD
        ...
        updates = UpdateRule.Adam(grads, learning_rate)
    else:
        raise Exception("Updater not understood: ",updater)

    update_model = theano.function([x, mask, dmask, learning_rate], cost, updates=updates)

    evaluate_model = theano.function([x, mask, dmask], cost)
    ...
    if fit_model == 1: # FIT MODEL TO THE DATA IF FLAG RAISED
        ...
        ppl = numpy.exp(nll)
        print(' >> Epoch = {0} NLL = {1} log2(NLL) = {2} exp(NLL) = {3} '.format(0,nll,(nll/math.log(2)),ppl))
	...
	logfd.close()
        ...
        ce = numpy.mean(val_scores)
        ppl = numpy.exp(nll)
        print(' > FINAL.VALID: Avg.NLL = {0} Avg.BPC = {1} PPL = {2} '.format(nll,(nll/math.log(2)),ppl))
        ...
        ce = numpy.mean(val_scores)
        ppl = numpy.exp(nll)
        print(' > FINAL.TRAIN: Avg.NLL = {0} Avg.BPC = {1} PPL = {2} '.format(nll,(nll/math.log(2)),ppl))
...
if __name__ == '__main__':
    ...
    options, remainder = getopt.getopt(sys.argv[1:], '', ["seed=","trainfile=","dictfname=",
                        ...
                        "modelPath=","error_mark=","fit_model=","useLN=","drop_p=","grad_clip=","patience="])
    ...
    train_model(seed=seed,model=model,batch_size=batch_size, n_h=n_h, n_epochs=n_epochs,
                ...
                out_wght_sd=out_wght_sd,useLN=useLN,drop_p=drop_p,grad_clip=grad_clip,patience=patience)
